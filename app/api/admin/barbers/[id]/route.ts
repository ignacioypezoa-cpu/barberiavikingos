...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) { const auth = await requireAdmin(); if (auth.error) return auth.error; try { const id = (await params).id; const count = await prisma.appointment.count({ where: { barberId: id } }); if (count) return NextResponse.json(await prisma.barber.update({ where: { id }, data: { active: false } })); await prisma.barber.delete({ where: { id } }); return NextResponse.json({ ok: true }); } catch (error) { return apiError(error); } }
